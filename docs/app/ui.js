...
export function createApp({ api, user, deskLookup, storage }) {
  ...
  // ---- BOOK PREVIEW VIEW ----
  function renderBookPreview(resourceId, targetDates, existingOwn) {
    clear();
    panel.appendChild(el("h2", "dra-title", "Select Days to Book"));

    const checked = new Set(targetDates);

    // Select all / none
    const toggleRow = el("div", "dra-actions");
    ...
    const selectNoneBtn = el("button", "dra-btn dra-btn-secondary", "Select None");
    ...
    // Count
    const countEl = el("p", "dra-cancel-count", "Booking " + checked.size + " days.");
    panel.appendChild(countEl);

    function updateCount() {
      countEl.textContent = "Booking " + checked.size + " day" + (checked.size !== 1 ? "s" : "") + ".";
    }
    ...
    for (const day of targetDates) {
      ...
      if (month !== currentMonth) {
        ...
        const monthDate = new Date(day + "T12:00:00Z");
        const monthName = monthDate.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" });
        checkList.appendChild(el("p", "dra-month-label", monthName));
      }
      const item = el("div", "dra-res-item");
      const cb = document.createElement("input");
      cb.type = "checkbox";
    ...
    const actions = el("div", "dra-actions");
    actions.style.marginTop = "1rem";
    const confirmBtn = el("button", "dra-btn dra-btn-primary", "Book Selected Days");
    confirmBtn.addEventListener("click", () => {
      if (checked.size === 0) return;
      const selected = targetDates.filter((d) => checked.has(d));
      renderProgress(resourceId, selected, existingOwn);
    });
    actions.appendChild(confirmBtn);
